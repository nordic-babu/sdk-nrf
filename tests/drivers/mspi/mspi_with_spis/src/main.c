...
static const struct device *spis_dev[CONFIG_MSPI_DATA_LINES_MAX] = {
	LISTIFY(CONFIG_MSPI_DATA_LINES_MAX, SPIS_DEV_ENTRY, (,))
};
static struct k_poll_signal async_sig[CONFIG_MSPI_DATA_LINES_MAX];
static struct k_poll_event async_evt[CONFIG_MSPI_DATA_LINES_MAX];
 ...
 */
static uint8_t rx_arr[CONFIG_MSPI_DATA_LINES_MAX]
...
static const struct spi_buf rx_buf[] = {
	LISTIFY(CONFIG_MSPI_DATA_LINES_MAX, RX_BUF_ENTRY, (,))
};
#define RX_SET_ENTRY(i, _) [i] = { .buffers = &rx_buf[i], .count = 1, }
static const struct spi_buf_set rx_set[] = {
	LISTIFY(CONFIG_MSPI_DATA_LINES_MAX, RX_SET_ENTRY, (,))
};
...
#if CONFIG_MSPI_DATA_LINES_MAX >= 2
...
#if CONFIG_MSPI_DATA_LINES_MAX >= 4
